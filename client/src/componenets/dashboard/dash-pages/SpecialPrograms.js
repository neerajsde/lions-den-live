...
import DisplaySpPg from '../sp-program-com/DisplaySpPg';
import AddSpPg from '../sp-program-com/AddSpPg';
import RemoveSppg from '../sp-program-com/RemoveSppg'
    ...
    <div className='w-full flex flex-col gap-4'>
        <div className="w-full flex items-center justify-between bg-[#222] p-3 rounded-sm !border !border-[#444]">
            <button
                ...
                >
                <IoHome className="text-xl" /> All Programs
            </button>
            <div className="flex items-center justify-center gap-4">
                ...
                >
                <TiDocumentAdd className="text-xl" /> Add Program
                </button>
                <button
                ...
                >
                <MdDelete className="text-xl" /> Update Program
                </button>
            </div>
        </div>
        <div className="w-full bg-[#111] p-3 rounded-sm !border !border-[#333]">
            {currentSection === '/' && (<DisplaySpPg/>)}
            {currentSection === 'add' && (<AddSpPg/>)}
            {currentSection === 'remove' && (<RemoveSppg/>)}
        </div>
    </div>
  )
}
