...
import SmSpinner from '../componenets/loader/SmSpinner';
    ...
    const loginHandler = async () => {
        ...
        try {
            ...
            if (response.ok && data.success === true) {
                if(data.admin){
                    setUser_id(data.admin_id);
                    setUserToken(data.token);
                    await verificationOtp(data.admin_id);
                    setIsVerifyOtp(true);
                } else{
                    toast.error('Not Logged In - This only for Admin');
                }
            } 
            else {
                if (data.tag === 'email') {
    ...
    const verifyOtp = async() => {
        try{
            ...
            if((otpCode === serverOpt) && (userToken !== '')){
                localStorage.setItem('lionDen', userToken);
                await AuthAdmin();
    ...
    <div className='w-full login min-h-screen bg-black flex flex-col items-center justify-center py-4'>
        <div className='w-[500px] flex flex-col text-white bg-[#111] py-6 px-8 max-sm:p-4 rounded-xl gap-2 max-sm:gap-4 max-md:w-[400px] max-sm:w-full'>
            <div className='w-full flex flex-col gap-1'>
                <h1 className='text-xl font-bold text-white max-md:text-lg'>{isVerifyOtp ? 'Verify Admin' : 'Admin-Login'}</h1>
            ...
            <div className='w-full flex flex-col gap-4 max-md:gap-3'>
                {!isVerifyOtp && (<label className='w-full flex flex-col justify-start'>
                ...
                {!isVerifyOtp && (<label className='w-full flex flex-col relative'>
                    <div className='w-full flex items-center justify-between'>
                        <p className='text-base max-md:text-sm font-semibold text-gray-500'>Password</p>
                        <p onClick={() => navigate('/forget-password')} className='text-md max-md:text-sm font-semibold text-yellow-500 cursor-pointer transition duration-200 ease-in hover:underline'>Lost Password?</p>
                    </div>
                    <input
                    ...
                    />
                    <div className='absolute top-12 right-2 text-3xl max-md:text-xl cursor-pointer text-gray-500' onClick={() => setIsVisiablePass(!isVisiablePass)}>
                ...
                {isVerifyOtp && (<label className='w-full flex flex-col gap-1'>
                    ...
                    <input
                        ...
                        className={`text-lg max-md:text-base max-sm:px-2 font-semibold py-2 px-4 bg-[#333] text-white placeholder:text-gray-500 rounded outline-none border-2 ${otpError.error ? 'border-red-500' : 'border-transparent transition duration-300 ease-in hover:border-yellow-300 focus:border-yellow-500'}`}
                    />
                    {
                        otpError.error && (
                            <div className='w-full flex items-center justify-start gap-2 text-red-500 text-sm font-medium max-sm:text-xs'>
                ...
                {!isVerifyOtp && (<div className='w-full flex justify-start'>
                    <button className='text-lg max-md:text-base flex items-center gap-4 font-extrabold text-white border-2 border-yellow-500 bg-yellow-500 py-1 px-4 rounded-3xl transition duration-300 ease-in hover:bg-transparent' onClick={loginHandler}>{isLoading && (<SmSpinner/>)}Sign in</button>
                </div>)}

                {isVerifyOtp && (<div className='w-full flex justify-start items-center gap-4'>
                    <button className='text-lg max-md:text-base flex items-center gap-4 font-extrabold text-white border-2 border-blue-500 bg-blue-500 py-1 px-4 rounded-3xl transition duration-300 ease-in hover:bg-transparent' onClick={(() => verificationOtp(user_id)) }>{isLoadingOTP && (<SmSpinner/>)}Resend</button>
                    <button className='text-lg max-md:text-base flex items-center gap-4 font-extrabold text-white border-2 border-yellow-500 bg-yellow-500 py-1 px-4 rounded-3xl transition duration-300 ease-in hover:bg-transparent' onClick={verifyOtp}>{isLoading && (<SmSpinner/>)}Verify otp</button>
                </div>)}
            </div>
            ...
            {!isVerifyOtp && (<div className='w-full flex items-center justify-start gap-2'>
                ...
                <p className='text-md max-md:text-sm font-semibold text-yellow-500 cursor-pointer transition duration-200 ease-in hover:underline' onClick={() => navigate('/admin-signup')}>Create account</p>
            </div>)}
        </div>
    </div>
  )
